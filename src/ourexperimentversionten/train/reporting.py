...
def resolve_results_path(connectivity_method: str, output_path: Path | None = None) -> Path:
    """Honor explicit paths; otherwise identify connectivity in the filename."""
    if output_path is not None:
        return Path(output_path)
    return (Path(__file__).resolve().parents[1] / "output" /
            f"BCI_IV_2a_GAT_{connectivity_method.upper()}_Results.json")


def print_subject_result(subject_number: int, results: dict[str, Any]) -> None:
...
def print_summary(subject_results: dict[int, dict[str, Any]]) -> None:
...
def save_results(subject_results: dict[int, dict[str, Any]], output_path: Path) -> None:

"""Tests for experiment five's CSD gamma-band wPLI LOSO data pipeline."""
...
from src.ourexperimentversionfive.data.csd_gamma_wpli import (
    NODE_VARIANT,
...
class CsdGammaWpliContractTests(unittest.TestCase):
...
class CsdGammaWpliLosoDataLoaderTests(unittest.TestCase):
    dataset: SavedDataset
    ...
    def test_edge_attr_matches_gamma_band_column(self) -> None:
        ...
        expected_gamma = np.asarray(
            self.dataset.edges["wpli_csd"][first_test_index, :, 4],
            dtype=np.float32,
        )
    ...
    def test_prevalidated_dataset_can_skip_redundant_full_scan(self) -> None:
        with patch(
            "src.ourexperimentversionfive.data.csd_gamma_wpli.validate_dataset"
        ) as validate:
            create_loso_dataloaders(
                test_subject_id=1,
                config=self.config,

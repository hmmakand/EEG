...
edges come from the ``wpli_without_csd`` variant, restricted to the gamma
band only (one scalar weight per directed electrode pair). This is the
gamma-band counterpart of :mod:`without_csd_alpha_wpli` -- same node/edge
variants, different band column. Graphs are returned as sparse
``torch_geometric.data.Data`` objects -- the saved dataset carries no
precomputed adjacency matrix, so there is nothing dense to threshold or
binarize.
"""
...
BAND_NAME: Final[str] = "gamma"


class _GammaWpliGraphSubset(Dataset):
    """Build sparse PyG graphs: without-CSD nodes, gamma-band wPLI edges.
    ...
    gamma-band column out of the 5-band edge array.
    """
    ...
    def get(self, idx: int) -> Data:
        ...
        if node_features.shape != (EXPECTED_NODES, EXPECTED_NODE_FEATURES):
            raise ValueError(
                "Unexpected node_features shape for the without-CSD gamma-wPLI "
        ...
        if not torch.isfinite(edge_attr).all():
            raise ValueError("Gamma-band edge weights contain non-finite values")
...
def create_loso_dataloaders(
    *,
    ...
    resolved_band_index = band_index(resolved_dataset, BAND_NAME)
    ...
    return LosoDataLoaderBundle(
        train=_make_loader(
            ...
            normalization,
            resolved_config,
            band_index=resolved_band_index,
            shuffle=True,
        ),
...
def _create_indexed_dataloaders(
    ...
    *,
    ...
    resolved_band_index = band_index(dataset, BAND_NAME)

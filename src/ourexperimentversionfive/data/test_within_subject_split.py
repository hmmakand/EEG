...
class CreateInnerValidationSplitTests(unittest.TestCase):
    def test_partition_covers_training_trials_exactly_once(self) -> None:
        subject_ids, labels = _synthetic_subject_ids_and_labels()
        folds = create_within_subject_folds(subject_ids, labels, 3, k=5, seed=1)
        fold = folds[0]

        inner_train, inner_validation = create_inner_validation_split(
        ...
        self.assertEqual(
            set(inner_train.tolist()) | set(inner_validation.tolist()),
            set(fold.train_graph_indices.tolist()),
        )
        self.assertFalse(set(inner_train.tolist()) & set(inner_validation.tolist()))

    def test_never_touches_the_fold_evaluation_trials(self) -> None:
        subject_ids, labels = _synthetic_subject_ids_and_labels()
        folds = create_within_subject_folds(subject_ids, labels, 3, k=5, seed=1)
        fold = folds[0]
        ...
        evaluation_set = set(fold.evaluation_graph_indices.tolist())
        self.assertFalse(evaluation_set & set(inner_train.tolist()))
        self.assertFalse(evaluation_set & set(inner_validation.tolist()))
    ...
    def test_is_reproducible_for_the_same_seed(self) -> None:
        ...
        first = create_inner_validation_split(
            fold.train_graph_indices, labels, validation_fraction=0.25, seed=7
        )
    ...
    def test_rejects_out_of_range_validation_fraction(self) -> None:
        with self.assertRaisesRegex(ValueError, "validation_fraction"):
            create_inner_validation_split(
                np.array([0, 1, 2, 3]), np.array([0, 1, 0, 1]), validation_fraction=0.0
            )
        with self.assertRaisesRegex(ValueError, "validation_fraction"):
            create_inner_validation_split(
                np.array([0, 1, 2, 3]), np.array([0, 1, 0, 1]), validation_fraction=1.0
            )

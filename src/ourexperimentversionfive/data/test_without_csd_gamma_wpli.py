"""Tests for experiment five's without-CSD gamma-band wPLI LOSO data pipeline."""
...
from src.ourexperimentversionfive.data.without_csd_gamma_wpli import (
    NODE_VARIANT,
...
class WithoutCsdGammaWpliContractTests(unittest.TestCase):
    ...
    def test_explicit_feature_contract_has_eight_without_csd_columns(self) -> None:
        self.assertEqual(len(NODE_FEATURE_NAMES), 8)
        self.assertEqual(NODE_FEATURE_NAMES[-1], "hjorth_complexity")


class WithoutCsdGammaWpliLosoDataLoaderTests(unittest.TestCase):
    dataset: SavedDataset
    ...
    def test_normalization_uses_training_pool_for_every_subject(self):
        ...
        raw = np.asarray(select_node_features(self.dataset, "without_csd", bundle.split.train_graph_indices), dtype=np.float64)
        ...
        raw_validation = select_node_features(self.dataset, "without_csd", first_index)
    ...
    def test_edge_attr_matches_gamma_band_column(self) -> None:
        bundle = create_loso_dataloaders(
            test_subject_id=1,
            config=self.config,
            dataset=self.dataset,
        ...
        expected_gamma = np.asarray(
            self.dataset.edges["wpli_without_csd"][first_test_index, :, 4],
            dtype=np.float32,
        )
    ...
    def test_prevalidated_dataset_can_skip_redundant_full_scan(self) -> None:
        with patch(
            "src.ourexperimentversionfive.data.without_csd_gamma_wpli.validate_dataset"
        ) as validate:
            create_loso_dataloaders(
                test_subject_id=1,
                config=self.config,

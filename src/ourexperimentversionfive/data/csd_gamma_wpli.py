"""Leakage-safe LOSO PyG DataLoaders for the CSD gamma-band wPLI combination.
...
restricted to the gamma band only. This is the gamma-band counterpart of
:mod:`csd_alpha_wpli` -- same signal source and connectivity method, different
band. Graphs are returned as sparse ``torch_geometric.data.Data`` objects,
matching the without-CSD combination.
"""
...
class _CsdGammaWpliGraphSubset(Dataset):
    """Build sparse PyG graphs: CSD nodes, gamma-band CSD wPLI edges.

    Reads ``nodes["csd"]``/``edges["wpli_csd"]``/``labels`` directly from the
    saved dataset's underlying memory maps and slices the gamma-band column
    ...
    def get(self, idx: int) -> Data:
        ...
        if node_features.shape != (EXPECTED_NODES, EXPECTED_NODE_FEATURES):
            raise ValueError(
                "Unexpected node_features shape for the CSD gamma-wPLI "
...
def _make_loader(
    ...
    *,
    ...
    subset = _CsdGammaWpliGraphSubset(
        dataset,
        graph_indices,
        normalization,

...
def create_inner_validation_split(
    ...
    *,
    ...
    """Split one fold's *training* trials into an inner train/validation pair.

    Used only for epoch selection (early stopping on the inner-validation
    split's loss) before the real fit on all of ``train_graph_indices`` --
    see ``training/within_subject.py``'s ``train_within_subject_fold``. This
    never sees a fold's evaluation trials, so it cannot leak into the
    reported held-out result; it exists because checkpointing on training
    loss alone always selects the most-overfit, latest epoch (training loss
    decreases near-monotonically), which was the direct cause of the
    within-subject diagnostic reporting chance-level accuracy despite >90%
    training accuracy.
    """

    if not 0 < validation_fraction < 1:
        raise ValueError("validation_fraction must be in the interval (0, 1)")
    graph_indices = np.asarray(train_graph_indices)
    if graph_indices.ndim != 1 or len(graph_indices) < 2:
        raise ValueError(
            "train_graph_indices must be a non-empty one-dimensional array "
            "with at least 2 trials"
        )
    fold_labels = np.asarray(labels)[graph_indices]

    splitter = StratifiedShuffleSplit(
        n_splits=1, test_size=validation_fraction, random_state=seed
    )
    inner_train_positions, inner_validation_positions = next(
        splitter.split(graph_indices, fold_labels)
    )
    inner_train_graph_indices = np.sort(graph_indices[inner_train_positions])
    inner_validation_graph_indices = np.sort(
        graph_indices[inner_validation_positions]
    )

    if set(map(int, inner_train_graph_indices)) & set(
        map(int, inner_validation_graph_indices)
    ):
        raise RuntimeError("Inner validation split has overlapping graph indices")
    if set(map(int, inner_train_graph_indices)) | set(
        map(int, inner_validation_graph_indices)
    ) != set(map(int, graph_indices)):
        raise RuntimeError(
            "Inner validation split does not exactly cover the fold's training trials"
        )

    return inner_train_graph_indices, inner_validation_graph_indices

...
from src.ourexperimentversionfive.model import (
    EEGGCN1,
...
class EEGGCN1ConfigTests(unittest.TestCase):
    def test_default_is_dedicated_to_the_without_csd_alpha_wpli_combination(self) -> None:
        ...
        self.assertEqual(EEGGCN1_INPUT_FEATURES, 8)
        self.assertEqual(EEGGCN1_NODES, 29)
        self.assertEqual(config.input_features, 8)
...
class EEGGCN1Tests(unittest.TestCase):
    ...
    def test_invalid_node_feature_shape_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, r"total_nodes, 8"):
...
class SelfOnlyTests(unittest.TestCase):
    def test_self_only_matches_nodewise_linear_and_ignores_connectivity(self):
        batch = _batch(0.0, 1.0)
        model = EEGGCN1(EEGGCN1Config(edge_mode="self_only"))
        model.eval()
        captured = []
        hook = model.convolution.register_forward_hook(
            lambda module, inputs, output: captured.append(output.detach().clone()))
        with torch.no_grad():
            first = model(batch.x, batch.edge_index, batch.edge_attr[:, 0], batch.batch)
            second = model(batch.x, batch.edge_index.flip(0),
                           batch.edge_attr[:, 0] * 0.01, batch.batch)
            expected = model.convolution.lin(batch.x) + model.convolution.bias
        hook.remove()
        torch.testing.assert_close(captured[0], expected)
        torch.testing.assert_close(first, second)
        self.assertEqual(sum(p.numel() for p in model.parameters()),
                         sum(p.numel() for p in EEGGCN1().parameters()))

    def test_invalid_edge_mode(self):
        with self.assertRaises(ValueError):
            EEGGCN1Config(edge_mode="invalid")


class LinearClassifierTests(unittest.TestCase):
    def test_linear_head_has_expected_capacity_and_trains(self):
        model = EEGGCN1(EEGGCN1Config(classifier="linear", edge_mode="self_only"))
        self.assertIsNone(model.classifier_hidden)
        self.assertEqual(sum(p.numel() for p in model.parameters()), 1106)
        self.assertEqual(sum(p.numel() for p in EEGGCN1().parameters()), 13721)
        batch = _batch(0.0, 1.0)
        output = model(batch.x, batch.edge_index, batch.edge_attr[:, 0], batch.batch)
        self.assertEqual(output.shape, (2, 2))
        torch.testing.assert_close(output.exp().sum(1), torch.ones(2))
        nn.NLLLoss()(output, torch.tensor([0, 1])).backward()
    ...
    def test_invalid_classifier(self):
        with self.assertRaises(ValueError):
            EEGGCN1Config(classifier="invalid")

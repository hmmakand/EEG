"""Train one or all LOSO folds on a selectable Graph-Liu2024-VersionTwo combination.

Which node/edge/band combination is trained is resolved at run time from
``TrainingConfig.combination`` via
``src.ourexperimentversionfive.data.combinations.get_combination`` -- swap
...
def _validate_normalization(
    ...
    """Check explicit disabled state or finite, positive shared training statistics."""

    if loaders.normalization.mode == "none":
        if loaders.normalization.mean_by_subject or loaders.normalization.standard_deviation_by_subject:
            raise ValueError("Disabled normalization must have no fitted statistics")
        return
...
def _save_fold_outputs(
    ...
    *,
    ...
    normalization_subjects = fold_subjects if loaders.normalization.mode == "zscore" else ()
    normalization = {
        "mode": loaders.normalization.mode,
        "epsilon": loaders.normalization.epsilon,
        "fit_graph_indices": list(loaders.normalization.fit_graph_indices),
        "fit_scope": "training_only" if loaders.normalization.mode == "zscore" else "disabled",
        "mean_by_subject": {
    ...
    _write_json(
        fold_dir / "result.json",
        {
            ...
            "normalization": {
                ...
                "fit_graph_indices": normalization["fit_graph_indices"],
                "mean_by_subject": {
...
def train_loso_fold(
    ...
    *,
    ...
    loader_config = GraphDataLoaderConfig(
        node_normalization=resolved_config.node_normalization,
    ...
    _log(
        f"Normalization   : {resolved_config.node_normalization} (training-only fitting when enabled)",
        enabled=show_progress,
    )
    _log(

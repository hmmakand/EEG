...
from src.ourexperimentversionfive.training import (
...
from src.ourexperimentversionfive.training.metrics import calculate_metrics
from src.ourexperimentversionfive.training.train import build_parser
...
def _graphs() -> PygDataLoader:
    ...
    graphs = [
        Data(
            x=torch.randn(29, 8, generator=generator) + graph_index % 2,
...
class TrainingUtilityTests(unittest.TestCase):
    ...
    def test_public_fold_training_uses_eight_features(self) -> None:
        result = train_loso_fold(
            1,
            TrainingConfig(
                epochs=1,
    ...
    def test_saved_fold_has_manifest_normalization_and_overwrite_protection(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(normalization["mean_by_subject"]), 0)
            for subject_mean in normalization["mean_by_subject"].values():
                self.assertEqual(tuple(subject_mean.shape), (8,))
            for subject_std in normalization["standard_deviation_by_subject"].values():
                self.assertEqual(tuple(subject_std.shape), (8,))

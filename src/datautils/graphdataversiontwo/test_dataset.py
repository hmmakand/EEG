...
class SavedDatasetTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        for variant_index, filename in enumerate(NODE_FILES.values()):
            values = np.full((n_samples, 29, 8), -20 + variant_index, dtype=np.float32)
            values[:, :, 5] = 0.5
            values[:, :, 6] = 0.2
            values[:, :, 7] = 1.5
    ...
    def test_rejects_old_schema_and_wrong_feature_order(self):
        for key, value in (("schema_version", 1), ("node_feature_names", list(reversed(NODE_FEATURE_NAMES)))):
    ...
    def test_saved_entropy_and_hjorth_corruption(self):
        filename = self.path / NODE_FILES["without_csd"]
        original = np.load(filename)
        for column, value in ((5, 1.5), (6, 0), (6, -1), (7, -1), (7, np.nan)):
            with self.subTest(column=column, value=value):
                corrupted = original.copy()
                corrupted[0, 0, column] = value
                np.save(filename, corrupted)
                with self.assertRaises(ValueError):
                    validate_saved_dataset(self.path)
        np.save(filename, original[:, :, :6])
    ...
    def test_graph_combines_variants_in_requested_column_order(self) -> None:
        ...
        self.assertEqual(graph["x"].shape, (29, 8))

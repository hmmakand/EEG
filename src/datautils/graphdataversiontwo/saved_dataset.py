...
from .config import NODE_FEATURE_NAMES, SCHEMA_VERSION
...
class SavedDataset:
    ...
    nodes : mapping of str to numpy.ndarray
        ``without_csd`` and ``csd`` arrays of shape ``(N, 29, 8)``.
    ...
    def get_graph(
        ...
        *,
        ...
        node_variant : {"without_csd", "csd"}
            Source for the eight node features, retaining metadata units.
        ...
        dict or torch_geometric.data.Data
            ``x`` has shape ``(29, 8)``, ``edge_index`` has shape ``(2, 812)``,
...
def load_dataset(path: str | Path) -> SavedDataset:
    ...
    if type(metadata.get("schema_version")) is not int or metadata["schema_version"] != SCHEMA_VERSION:
        raise ValueError("Unsupported schema_version: version two requires schema 2 (eight features); use version one's loader for schema 1")
    if metadata["node_feature_names"] != list(NODE_FEATURE_NAMES):
        raise ValueError("metadata node_feature_names must match the eight-feature names and order")
    ...
    for field, expected in (("channel_names", 29), ("node_feature_names", len(NODE_FEATURE_NAMES)), ("band_names", 5)):
    ...
    expected_shapes = {
        **{filename: (n_samples, 29, len(NODE_FEATURE_NAMES)) for filename in NODE_FILES.values()},
...
def get_graph(
    ...
    *,
    ...
    node_variant : {"without_csd", "csd"}
        Source for the eight electrode features.
    edge_variants : sequence of str

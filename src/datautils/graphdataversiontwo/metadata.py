...
def generation_settings():
    ...
    return {
        ...
        'hjorth': {'input': 'Per-electrode original trial for each EEG/CSD variant; no additional band filter',
                   'difference_orders': [1, 2], 'variance_ddof': 0,
                   'sampling_rate_multiplier': False,
                   'mobility': 'sqrt(var(diff(x))/var(x))',
                   'complexity': 'sqrt(var(diff(x,2))/var(diff(x)))/mobility',
                   'invalid_input': 'Reject nonfinite input, fewer than 3 samples, zero signal or first-difference variance'},
        ...
        'node_units': {
            'without_csd': {'signal': 'V', 'linear_power': 'V^2', 'db_reference': '1 microvolt^2',
                    ...
                    'hjorth_mobility': 'per sample', 'hjorth_complexity': 'dimensionless'},
...
def build_dataset_metadata(n_samples, subjects, montage, subject_records, arrays, fingerprint,
    ...
    return {
        ...
        **({'expected_class_counts_per_subject': {'left_hand': 20, 'right_hand': 20}}
        ...
        'source_notebook': 'src/datautils/graphdataversiontwo/readmoabbliu.ipynb',

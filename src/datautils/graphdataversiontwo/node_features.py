"""Calculate the eight inspected features from EEG or surface-Laplacian trials."""
...
def compute_hjorth_features(trial: np.ndarray) -> np.ndarray:
    """Return per-channel mobility and complexity, shape (n_channels, 2).

    Input is one EEG (V) or CSD (V/m²) trial, channels by samples.
    Float64 population variances (ddof=0) use first/second sample differences,
    without sampling-rate multiplication or additional filtering. Mobility is
    per sample; complexity is dimensionless. Inputs are never modified.
    Reject nonfinite input, fewer than three samples, or undefined ratios,
    identifying zero-based channel indices for degenerate signals.
    """
    signal = np.asarray(trial, dtype=np.float64)
    if signal.ndim != 2 or signal.shape[0] == 0 or signal.shape[1] < 3:
        raise ValueError("Hjorth input requires channels × samples with at least three samples")
    if not np.isfinite(signal).all():
        raise ValueError("Hjorth input contains nonfinite samples")
    v0 = np.var(signal, axis=-1, ddof=0)
    v1 = np.var(np.diff(signal, axis=-1), axis=-1, ddof=0)
    v2 = np.var(np.diff(signal, n=2, axis=-1), axis=-1, ddof=0)
    invalid = (v0 <= 0) | (v1 <= 0)
    if invalid.any():
        raise ValueError(f"Undefined Hjorth ratios for channel indices {np.flatnonzero(invalid).tolist()}")
    mobility = np.sqrt(v1 / v0)
    complexity = np.sqrt(v2 / v1) / mobility
    features = np.column_stack((mobility, complexity))
    if not np.isfinite(features).all():
        raise ValueError("Hjorth features contain nonfinite values")
    return features


def build_node_features(trial: np.ndarray, sfreq: float) -> np.ndarray:
    """Combine five log-band powers, spectral entropy, and two Hjorth descriptors.
    ...
    Returns
    -------
    features : ndarray of float64, shape (n_channels, 8)
        Delta, theta, alpha, beta, and gamma powers in dB, followed by
        dimensionless spectral entropy, mobility (per sample), and complexity
        (dimensionless). EEG powers use a reference of
    ...
    features = np.column_stack([log_band_powers, normalized_entropy, compute_hjorth_features(trial)])
    if not np.isfinite(features).all():

...
| `data/moabb/Graph-Liu2024-VersionOne/` | Output directory to create for the saved feature dataset |
| `src/datautils/graphdataversionone/` | Python generation code, loader, notebook, and plan |
...
Keep two node variants: calculate the same six features from EEG **without CSD** and from EEG **after CSD**:
...
Apply CSD to the time series before calculating the CSD-derived features. Both variants retain the same electrode nodes and feature order, with shape **`(29, 6)`** per trial.
...
All files below belong in `data/moabb/Graph-Liu2024-VersionOne/`. `N` is the number of saved trials.

| File | Contents |
| --- | --- |
| `node_features_without_csd.npy` | Six node features from EEG without CSD, `(N, 29, 6)` |
| `node_features_csd.npy` | Six node features from CSD-transformed EEG, `(N, 29, 6)` |
...
| `node_features.py` | `compute_psd()`, `compute_band_powers()`, `compute_spectral_entropy()`, `build_node_features()` — produce the six node features for either signal variant, as planned |

...
def validate_features(
    ...
    *,
    ...
    node_features : mapping of str to numpy.ndarray
        ``without_csd`` and ``csd`` arrays of shape ``(n_channels, 6)``.
        The first five columns are dB band powers, the last is normalized
        spectral entropy. Power references/units are recorded in metadata.
    ...
    n_node_features : int
        Expected node width; entropy is always the last column.
    n_bands : int

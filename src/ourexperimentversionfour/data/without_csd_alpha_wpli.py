...
def fit_feature_normalization(
    ...
    *,
    ...
    only from its own ``without_csd`` node features -- independent of which
    LOSO fold or split a subject ends up in, so this can be (and is) fit once
    per dataset rather than once per fold.

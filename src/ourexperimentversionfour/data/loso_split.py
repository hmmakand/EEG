...
class FeatureNormalization:
    """Per-subject z-scoring statistics: one mean/std pair per subject ID.

    Each subject's own raw node features (never labels) fix that subject's
    statistics, so this removes subject-specific baseline shifts (skull
    thickness, electrode contact, alertness, ...) the same way for every
    split -- train, validation, and the held-out test subject alike. This is
    the standard cross-subject EEG/BCI alignment practice (e.g. Euclidean
    Alignment): using a new subject's own unlabeled signal to recenter their
    features is available at real deployment time, so it is not leakage.
    """

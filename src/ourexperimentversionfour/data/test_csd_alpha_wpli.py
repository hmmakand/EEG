"""Tests for experiment four's CSD alpha-band wPLI LOSO data pipeline."""
...
from src.ourexperimentversionfour.data.validation import (
    EXPECTED_EDGES,
    EXPECTED_GRAPHS,
    EXPECTED_NODES,
...
class CsdAlphaWpliContractTests(unittest.TestCase):
    ...
    def test_explicit_feature_contract_has_six_csd_columns(self) -> None:
...
class CsdAlphaWpliLosoDataLoaderTests(unittest.TestCase):
    ...
    def test_normalization_is_per_subject_using_only_that_subjects_own_nodes(self) -> None:
        ...
        raw_subject = np.asarray(
            self.dataset.nodes["csd"][subject_ids == one_train_subject],
            dtype=np.float64,
        )
    ...
    def test_prevalidated_dataset_can_skip_redundant_full_scan(self) -> None:
        with patch(
            "src.ourexperimentversionfour.data.csd_alpha_wpli.validate_dataset"
        ) as validate:
            create_loso_dataloaders(
                test_subject_id=1,
                config=self.config,

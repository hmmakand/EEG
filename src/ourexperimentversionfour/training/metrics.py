"""Classification metrics for graph-level predictions."""
...
class ClassificationMetrics:
    ...
    ``balanced_accuracy`` (mean per-class recall) and ``cohens_kappa``
    (chance-corrected agreement) are macro/chance-corrected instead, since
    the fixed 20/20 class split per subject motivates going beyond raw
    accuracy for LOSO reporting.
    """
...
def calculate_metrics(
    *,
    ...
    return ClassificationMetrics(
        ...
        cohens_kappa=float(cohen_kappa_score(targets, predicted)),
